 ...
 * @brief Graptoς editor tab hover refs module.
...
void hide_color_preview(EditorTab *tab) {
    ...
    if (tab->hover_popover) graptos_popover_hide(tab->hover_popover);
}
...
gboolean hover_transition_timeout_cb(gpointer user_data) {
     ...
     * delay.
     */
    if (tab->regex_tester_active) return G_SOURCE_REMOVE;
...
static void schedule_reference_lookup_at_iter(EditorTab *tab,
    ...
    tab->hover_word = word;
    gint lsp_line = 0;
    gint lsp_character = 0;
    lsp_iter_to_position(iter, &lsp_line, &lsp_character);
    tab->hover_lsp_line = lsp_line >= 0 ? (guint)lsp_line : 0u;
    tab->hover_lsp_character = lsp_character >= 0 ? (guint)lsp_character : 0u;
    tab->hover_lsp_position_valid = TRUE;
...
void editor_tab_show_reference_at_pointer_or_cursor(EditorTab *tab) {
    ...
    if (tab->pointer_valid) {
        int buffer_x = 0;
        int buffer_y = 0;
        gtk_text_view_window_to_buffer_coords(GTK_TEXT_VIEW(tab->text_view),
...
void hover_clear_rows(EditorTab *tab) {
    ...
    graptos_list_box_clear(tab->hover_list);
}
...
void hover_row_activated(GtkListBox *box,
     ...
     */
    const char *path = g_object_get_data(G_OBJECT(row), "graptos-ref-path");
