...
static CleafGitErrorKind classify_git_error(const char *out, const char *err) {
    ...
    if (contains_ascii(text, "not a git repository")) return CLEAF_GIT_ERROR_NOT_REPO;
    ...
    if (contains_ascii(text, "no configured push destination")
        ...
        return CLEAF_GIT_ERROR_NO_REMOTE;
    }
...
static const char *git_error_title(CleafGitErrorKind kind) {
    ...
    case CLEAF_GIT_ERROR_AUTH: return "Git authentication failed";
    case CLEAF_GIT_ERROR_NO_REMOTE: return "Git remote is not configured";
    case CLEAF_GIT_ERROR_NETWORK: return "Git network error";
    case CLEAF_GIT_ERROR_CONFLICT: return "Git conflict";
    case CLEAF_GIT_ERROR_LOCAL_CHANGES: return "Local changes block this Git command";
    case CLEAF_GIT_ERROR_NON_FAST_FORWARD: return "Git non-fast-forward error";
    case CLEAF_GIT_ERROR_DETACHED_HEAD: return "Git detached HEAD";
    case CLEAF_GIT_ERROR_NOTHING_TO_COMMIT: return "Nothing to commit";
    case CLEAF_GIT_ERROR_GIT_MISSING: return "Git executable was not found";
    case CLEAF_GIT_ERROR_NONE: return "Git command completed";
    case CLEAF_GIT_ERROR_OTHER:
...
static char *current_repo(EditorWindow *win, char **rel_file_out) {
    ...
    if (tab && tab->file_path) {
        char *repo = cleaf_git_repo_for_path(tab->file_path);
        if (repo) {
    ...
    if (win && win->project_root) {
        return cleaf_git_repo_for_path(win->project_root);
    }
    if (project_root_count(win) > 0u) {
        return cleaf_git_repo_for_path(project_root_at(win, project_root_count(win) - 1u));
    }
    return NULL;
}
...
void cleaf_git_refresh_all(EditorWindow *win) {
    ...
    for (guint i = 0u; i < count; i++) {
        ...
        char *repo = cleaf_git_repo_for_path(root);
